...
//            }
    ...
    public static void main(String[] args) throws IOException {
        ...
        obj.put("title", "消息平台标题");
        obj.put("content", "消息平台restful测试");
        List<String> list = new ArrayList<>();
        list.add("7020160452");//冯广亮
        list.add("2012000408");//张翰青
        obj.put("users", list);
        obj.put("sendByEmail", true);
        obj.put("sendByWechat", false);
        obj.put("sendBySms", false);
        String url = "http://localhost:8080/mht_ump/a/ump/sendMessage/send";

        RestfulHttpClient.HttpResponse response = getClient(url)
                             //设置post请求
//                .addHeader("token", "tokenValue")   //添加header
//                .addPathParam("id", "2")        //设置url路径参数
//                .addPostParam("title", "消息平台标题") //设置url请求参数
//                .addPostParam("content", "消息平台restful测试") //设置url请求参数
//                .addPostParam("sendByEmail", "true") //设置url请求参数
//                .addPostParam("sendByWechat", "false") //设置url请求参数
//                .addPostParam("sendBySms", "false") //设置url请求参数
//                .addPostParam("users", list.toString()) //设置url请求参数
